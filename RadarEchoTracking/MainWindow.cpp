	...
	, m_CurrentReplayFileIndex(1)
{
	...
	m_FileNameDockList->m_Setup.listView->setSelectionMode(QAbstractItemView::ExtendedSelection);
	//////////////////////////////////////////////////////////////////////////
...
void MainWindow::slot_ActionTriggered( QAction* action )
{
	...
	else if(action == m_pSectionLineAction)
	{
		...
		if(bChecked)
		{
			...
			{
				m_pDrawClipLine->SetFileName(m_OpenFileName);
			}
			else
			{
			...
			m_pDrawClipLine->SetLineMode(false);
	...
	else if(action == m_pDistanctTestAction)
	{
		...
		if(bChecked)
		{
			...
			m_pDrawClipLine->m_KeySwitch = m_keySwitchMaipulator;
			m_pDrawClipLine->SetLineMode(true);
			m_keySwitchMaipulator->selectMatrixManipulator(2);
		}
		else
		{
	...
	else if(m_pOpenFlightPathAction == action)
	{
		...
		{
			LoadFlightPath(strFileName);


			static FlightAndRasterWindow* pRasterWindow = 0;
			if(pRasterWindow == 0)
			{
				pRasterWindow = new FlightAndRasterWindow(m_FlightPathControler, m_pControlorInterface);
				pRasterWindow->SetMapToFilePath(&m_FileBaseMapToFilePath);

				connect(this, SIGNAL(signal_SelectFiles(const QStringList&)), pRasterWindow, SLOT(slot_updateSelectNcFiles(const QStringList&)));
			}
			
			QStringList ncFilesList;
			ncFilesList.push_back(m_FileList[0]);
			ncFilesList.push_back(m_FileList[m_FileList.size() - 1]);
			pRasterWindow->slot_updateSelectNcFiles(ncFilesList);
			pRasterWindow->show();

			pRasterWindow->FillList();
...
void MainWindow::LoadBusinessFeature()
{
	...
	m_pMap->Load2DMapFromConfig(strMapConfig.toStdString());
...
void MainWindow::StartTrack()
{
	...
	m_pControlorInterface->LoadNcFile(strFilePath.toStdString());
	m_pControlorInterface->DisplayNcFile(strFilePath.toStdString());
...
void MainWindow::DisplayRadarData(const QString& strFile, const QIcon* pIcon)
{
	...
	m_pControlorInterface->DisplayNcFile(strFile.toStdString());
}
...
void MainWindow::slot_TrackDone(const QString& doneFile)
{
	...
	{
		m_pControlorInterface->AddRect(m_RectResultSet.at(i).name.toStdString()
			...
			, (osg::Vec4)m_RectResultSet.at(i).color, strBaseName.toStdString());
	}
...
void MainWindow::ChangeRectColor(const QString& strBaseName )
{
	...
	for(int i = 0; i < selectBoxes.size(); i++)
	{
		...
		if(iIndex > 0)
		{
			m_pControlorInterface->SetRectColor(cd.name.toStdString(), iIndex, osg::Vec4(0, 0, 0, 1));
		}
	}
...
void MainWindow::slot_Replay( const QString& doneFile )
{
	...
	for(int i = 0; i < iRectCount; i++)
	{
		...
		if(iIndex > 0)
		{
			...
			std::string strName = m_RectResultSet.at(i).name.toStdString();
...
{
	m_pControlorInterface->DeleteNcFile(preFile.toStdString());
}
...
int MainWindow::GetRealTrackFileID( QPair<QString, QString>& filePair )
{
	if(m_CurrentRealFileIndex < m_SelectFilesList.size())
	{
		filePair.first = m_SelectFilesList.at(m_CurrentRealFileIndex - 1);
		filePair.second = m_SelectFilesList.at(m_CurrentRealFileIndex);
		m_CurrentRealFileIndex++;
		return 0;
	}
	else
	{
		if(m_FileList.size() < 0) return -1;
		static QString curFileBaseName = m_SelectFilesList.at(m_SelectFilesList.size() - 1);
...
void MainWindow::FlushCubesList()
{
	...
	for(int i = 0; i < myCubeData.size(); i++)
	{
		...
		m_pControlorInterface->AddRect(strName.toStdString(), p1, p2, p3, p4, osg::Vec4(.0, 0.0, 0.0, 1.0));
		m_pControlorInterface->HideRects(strName.toStdString(), true);
	}
}
...
void MainWindow::slot_doubleClicked( const QModelIndex& index )
{
	...
	if(pItem)
	{
		...
		if(pItem->background().color() == QColor(0, 255, 0))
		{
...
{
	QDateTime currDateTime = QDateTime::fromString("20" + text, "yyyyMMddhhmm");
	currDateTime.setTimeSpec(Qt::UTC);
...
void MainWindow::LoadPaoDian()
{
	...
	PaoDinaControlerInterface* pPaoDinaControlerInterface = 
		(PaoDinaControlerInterface*)(pLayer->GetDataProvider()->GetExtendInterface(0));

	m_PaoDianDockWidget->LoadSlot(pPaoDinaControlerInterface->GetModel());
	m_PaoDianDockWidget->SetPaoDinaControlerInterface(pPaoDinaControlerInterface);

	pPaoDinaControlerInterface->HideAll();

}

void MainWindow::LoadFlightPath(const QString& fileName)
{
...
__CHECK__: 
		...
		{
			Sleep(800);
			m_pMainWindow->m_pControlorInterface->LoadNcFile(strFilePath2.toStdString());
		...
		else
		{
			for(int ii = 0; ii < cubes.size(); ii++)
			{
				...
				P_TREC_NC_MAP_OUT pOut = m_pMainWindow->m_pControlorInterface->runTrec(strFilePath1.toStdString()
					, strFilePath2.toStdString(), cd);
			
				qDebug()<<"---------------------end-----------------------";
	#else
				P_TREC_NC_MAP_OUT pOut = m_pMainWindow->Test(strFilePath1.toStdString()
					, strFilePath2.toStdString(), cd);
	#endif
				/////////////////////////////////handle reslut/////////////////////////////////////////
				//m_pMainWindow->Lock();
				...
				////////////////////////////////////获得跟踪框中的回波强度图像//////////////////////////////////////
				QImage* pImage = m_pMainWindow->m_pControlorInterface->ClipNcFile(strFilePath2.toStdString(), osg::Vec3(fTargetFstLon, fTargetLstLat, 0), osg::Vec3(fTargetLstLon, fTargetFstLat, 0), 0);
				
				QImage* pSectionImage = m_pMainWindow->m_pControlorInterface->GetSectionNcFile(strFilePath2.toStdString(), osg::Vec3(fTargetFstLon, fTargetLstLat, 0), osg::Vec3(fTargetLstLon, fTargetFstLat, 0));
